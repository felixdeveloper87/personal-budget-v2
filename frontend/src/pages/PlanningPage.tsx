    ...
    <Box minH="100vh" maxW="appContent" mx="auto" px={{ base: 2, md: 4, lg: 6 }} py={{ base: 4, md: 7 }}>
      <MotionBox variants={containerV} initial="hidden" animate="show">
        <VStack align="stretch" spacing={{ base: 4, md: 5 }}>
          ...
          <MotionBox variants={riseV}>
            {currentMonthLoading ? <Box h="250px" borderRadius="22px" bg="var(--pb-surface-2)" /> : <PlanningHero monthlyPositive={monthlyPositive} balance={currentMonthData.balance} income={currentMonthData.income} expense={currentMonthData.expense} forecast={forecast} firstNegativeMonth={firstNegativeMonth} onOpenGoals={() => onPageChange?.('goals')} />}
          </MotionBox>
          ...
          <MotionBox variants={riseV}>
            <PlanPanel eyebrow="Projection" title="Your 12-month runway" caption={forecast?.hasProjectionBasis ? `Based on recent activity from ${basisLabel}, scheduled commitments and your income assumption.` : 'Add transaction history, fixed payments or instalments to make this forecast more useful.'} rightSlot={forecast?.months.length ? <HStack spacing={2}><ForecastNav label="Previous forecast month" icon={ChevronLeft} onClick={() => scrollForecast(-1)} /><ForecastNav label="Next forecast month" icon={ChevronRight} onClick={() => scrollForecast(1)} /></HStack> : undefined}>
...
function PlanningHero({ monthlyPositive, balance, income, expense, forecast, firstNegativeMonth, onOpenGoals }: { monthlyPositive: boolean; balance: number; income: number; expense: number; forecast: CashFlowForecast | null; firstNegativeMonth: CashFlowForecastMonth | undefined; onOpenGoals: () => void }) {
  const projected = forecast?.months[0]?.projectedClosingBalance
  return <Box overflow="hidden" position="relative" borderRadius="22px" bg="linear-gradient(125deg, var(--pb-forest) 0%, var(--pb-forest-2) 58%, var(--pb-line) 100%)" color="#f6f8fb" boxShadow="var(--pb-shadow-lift)" p="clamp(1.25rem, 4vw, 2rem)"><Box position="absolute" w="340px" h="340px" border="1px solid rgba(255,255,255,.15)" borderRadius="full" right="-85px" top="-190px" /><Box position="absolute" w="230px" h="230px" border="1px solid rgba(255,255,255,.10)" borderRadius="full" right="90px" bottom="-175px" /><Flex position="relative" zIndex={1} direction={{ base: 'column', lg: 'row' }} justify="space-between" gap={6}><Box maxW="620px"><HStack spacing={2} mb={3}><Flex w={7} h={7} align="center" justify="center" borderRadius="full" bg={monthlyPositive ? 'rgba(95,211,148,.20)' : 'rgba(246,146,138,.20)'}><Icon as={monthlyPositive ? CheckCircle2 : AlertTriangle} boxSize={4} weight="fill" /></Flex><Text fontFamily="var(--pb-mono)" fontSize="10px" letterSpacing="0.18em" textTransform="uppercase" opacity={0.76}>{monthlyPositive ? 'Month on track' : 'Attention needed'}</Text></HStack><Text fontSize={{ base: '2xl', md: '3xl' }} fontWeight={500} lineHeight="1.08" letterSpacing="-0.03em">{monthlyPositive ? 'Your income is covering this month’s plan.' : 'This month needs a recovery plan.'}</Text><Text fontSize="sm" lineHeight="1.55" opacity={0.82} mt={3}>{monthlyPositive ? `${money(income)} coming in against ${money(expense)} going out leaves ${money(balance)} to protect or allocate.` : `${money(expense)} in expenses is ${money(Math.abs(balance))} above income. Rebalance a limit or create a break-even target.`}</Text>{!monthlyPositive && <Button mt={5} size="sm" rightIcon={<Icon as={ArrowRight} boxSize={4} />} onClick={onOpenGoals} bg="#f6f8fb" color="var(--pb-forest)" _hover={{ bg: 'white' }}>Open break-even target</Button>}</Box><SimpleGrid columns={2} spacing={{ base: 3, md: 5 }} minW={{ lg: '330px' }}><HeroMetric label="This month" value={signedMoney(balance)} /><HeroMetric label="Next month" value={projected === undefined ? '—' : money(projected)} note={projected !== undefined ? 'Projected closing balance' : undefined} /><HeroMetric label="Current balance" value={forecast ? money(forecast.currentTotalBalance) : '—'} /><HeroMetric label="Runway" value={firstNegativeMonth ? monthLabel(firstNegativeMonth.month) : '12 months+'} note={firstNegativeMonth ? 'First negative month' : 'Plan remains positive'} /></SimpleGrid></Flex></Box>
}
...
function Signal({ icon, label, value, note, accent = 'brand' }: { icon: typeof Wallet; label: string; value: string; note: string; accent?: 'brand' | 'income' | 'expense' }) {
...
function ForecastCard({ month, variableLabel, hasIncomePlan }: { month: CashFlowForecastMonth; variableLabel: string; hasIncomePlan: boolean }) { const confidence = month.confidencePercent; const colour = month.negative ? 'var(--pb-coral)' : 'var(--pb-ink)'; return <Box data-forecast-card flex={{ base: '0 0 88%', sm: '0 0 62%', lg: '0 0 calc(33.333% - 8px)' }} minW={0} p={4} borderRadius="16px" scrollSnapAlign="start" bg="var(--pb-surface-2)" border="1px solid" borderColor={month.negative ? 'var(--pb-tint-coral)' : 'var(--pb-hair)'}><Flex justify="space-between" align="start" gap={3}><Box><Text fontWeight={600} color="var(--pb-ink)">{monthLabel(month.month)}</Text><Text fontFamily="var(--pb-mono)" fontSize="9px" letterSpacing="0.12em" textTransform="uppercase" color="var(--pb-ink-faint)" mt="4px">{confidence}% committed</Text></Box><Box textAlign="right"><Text className="num" fontSize="lg" fontWeight={600} color={colour} style={{ fontVariantNumeric: 'tabular-nums' }}>{money(month.projectedClosingBalance)}</Text><Text fontSize="xs" color={month.netCashFlow >= 0 ? 'var(--pb-income)' : 'var(--pb-coral)'} mt="2px">{signedMoney(month.netCashFlow)} net</Text></Box></Flex><SimpleGrid columns={2} spacing={3} mt={4}><Box><Text fontFamily="var(--pb-mono)" fontSize="9px" letterSpacing="0.12em" textTransform="uppercase" color="var(--pb-ink-faint)">Income</Text><Text fontSize="sm" fontWeight={600} color="var(--pb-income)" mt="2px">{money(month.fixedIncome + month.estimatedIncome)}</Text><Text fontSize="10px" color="var(--pb-ink-soft)" mt="2px">Recurring + {hasIncomePlan ? 'planned' : 'estimated'}</Text></Box><Box><Text fontFamily="var(--pb-mono)" fontSize="9px" letterSpacing="0.12em" textTransform="uppercase" color="var(--pb-ink-faint)">Outgoings</Text><Text fontSize="sm" fontWeight={600} color="var(--pb-coral)" mt="2px">{money(month.installmentExpense + month.fixedExpense + month.estimatedVariableExpense)}</Text><Text fontSize="10px" color="var(--pb-ink-soft)" mt="2px">Fixed + {variableLabel}</Text></Box></SimpleGrid><Box h="5px" mt={4} bg="var(--pb-surface-3)" borderRadius="full" overflow="hidden"><Box h="full" w={`${confidence}%`} bg={confidence >= 66 ? 'var(--pb-income-2)' : confidence >= 33 ? 'var(--pb-gold-2)' : 'var(--pb-ink-faint)'} borderRadius="full" /></Box></Box> }
