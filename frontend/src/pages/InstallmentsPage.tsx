      ...
      <MotionBox variants={containerV} initial="hidden" animate="show">
        <VStack align="stretch" spacing={{ base: 3, md: 3.5 }}>
          ...
          {loading ? <Flex justify="center" py={20}><Spinner color="var(--pb-forest-2)" /></Flex> : <>
            ...
            <SimpleGrid columns={{ base: 1, sm: 3 }} spacing="0.6rem">
              <MotionBox variants={riseV}><Metric icon={CreditCard} label="Active plans" value={String(summary.active.length)} /></MotionBox>
              <MotionBox variants={riseV}><Metric icon={Wallet} label="Remaining" value={money(summary.remaining)} accent="brand" /></MotionBox>
              <MotionBox variants={riseV}><Metric icon={CheckCircle2} label={`Paid · ${summary.completed.length} done`} value={money(summary.paid)} accent="income" /></MotionBox>
            </SimpleGrid>
...
function InstallmentOutlook({ months, carouselRef, onNavigate }: { months: Array<{ date: Date; total: number }>; carouselRef: RefObject<HTMLDivElement>; onNavigate: (direction: -1 | 1) => void }) {
  ...
  return <Box p={{ base: 2.5, md: 3 }} borderRadius="14px" bg="var(--pb-surface)" border="1px solid var(--pb-hair)" boxShadow="var(--pb-shadow)"><Flex justify="space-between" align="center" gap={3} mb={2.5}><HStack spacing={2.5} align="baseline" minW={0}><Text fontFamily="var(--pb-mono)" fontSize="8.5px" letterSpacing="0.16em" textTransform="uppercase" color="var(--pb-forest-2)" whiteSpace="nowrap">Outlook</Text><Text fontSize="sm" fontWeight={600} color="var(--pb-ink)" noOfLines={1}>Your next six months</Text></HStack><HStack spacing={1.5} flexShrink={0}><OutlookNav label="Previous month" icon={ChevronLeft} onClick={() => onNavigate(-1)} /><OutlookNav label="Next month" icon={ChevronRight} onClick={() => onNavigate(1)} /></HStack></Flex><Box ref={carouselRef} onPointerDown={startDrag} onPointerMove={moveDrag} onPointerUp={stopDrag} onPointerCancel={stopDrag} onLostPointerCapture={() => setIsDragging(false)} display="flex" gap={2} overflowX="auto" pb={1} cursor={isDragging ? 'grabbing' : 'grab'} userSelect={isDragging ? 'none' : 'auto'} scrollSnapType={isDragging ? 'none' : 'x mandatory'} sx={{ touchAction: 'pan-y', WebkitOverflowScrolling: 'touch', scrollbarWidth: 'none', '&::-webkit-scrollbar': { display: 'none' } }}>{months.map((month, index) => <Box key={month.date.toISOString()} data-installment-month flex={{ base: '0 0 46%', sm: '0 0 calc(33.333% - 6px)', lg: '0 0 calc(25% - 6px)' }} px={2.5} py={2} borderRadius="11px" bg={index === 0 ? 'var(--pb-tint-green)' : 'var(--pb-surface-2)'} border="1px solid" borderColor={index === 0 ? 'var(--pb-hair-2)' : 'var(--pb-hair)'} scrollSnapAlign="start"><Flex justify="space-between" align="baseline" gap={2}><Text fontFamily="var(--pb-mono)" fontSize="8px" letterSpacing="0.1em" textTransform="uppercase" color="var(--pb-ink-faint)" noOfLines={1}>{index === 0 ? 'This month' : monthName(month.date)}</Text><Text className="num" fontSize="sm" fontWeight={600} color="var(--pb-ink)" style={{ fontVariantNumeric: 'tabular-nums' }}>{money(month.total)}</Text></Flex><Box h="3px" mt={1.5} bg="var(--pb-surface-3)" borderRadius="full" overflow="hidden"><Box h="full" w={`${Math.max(4, (month.total / highest) * 100)}%`} bg="var(--pb-forest-2)" borderRadius="full" /></Box></Box>)}</Box></Box>
}
...
function Metric({ icon, label, value, accent = 'brand' }: { icon: typeof Wallet; label: string; value: string; accent?: 'brand' | 'income' }) { const colour = accent === 'income' ? 'var(--pb-income)' : 'var(--pb-forest-2)'; const tint = accent === 'income' ? 'var(--pb-tint-income)' : 'var(--pb-tint-green)'; return <Box h="full" px={3} py={2.5} borderRadius="12px" bg="var(--pb-surface)" border="1px solid var(--pb-hair)" boxShadow="0 1px 2px rgba(15,23,42,.04)"><Flex align="center" gap={2.5}><Flex w={8} h={8} align="center" justify="center" borderRadius="10px" bg={tint} color={colour} flexShrink={0}><Icon as={icon} boxSize={4} strokeWidth={1.8} aria-hidden="true" /></Flex><Box minW={0}><Text fontFamily="var(--pb-mono)" fontSize="8.5px" letterSpacing="0.14em" textTransform="uppercase" color="var(--pb-ink-faint)" noOfLines={1}>{label}</Text><Text className="num" fontSize="md" fontWeight={600} color="var(--pb-ink)" lineHeight="1.1" style={{ fontVariantNumeric: 'tabular-nums' }}>{value}</Text></Box></Flex></Box> }
