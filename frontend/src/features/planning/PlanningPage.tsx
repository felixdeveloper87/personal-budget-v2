    ...
    <Box minH="100vh" maxW="appContent" mx="auto" px={{ base: 2, md: 4, lg: 6 }} py={{ base: 3, md: 7 }}>
      <MotionBox variants={containerV} initial="hidden" animate="show">
        <VStack align="stretch" spacing={{ base: 3, md: 5 }}>
          ...
          <MotionBox variants={riseV}>
            <PlanPanel eyebrow="Projection" title="Your 12-month runway" caption="Starts from today’s live account balance and applies every dated future transaction, instalment and recurring payment in its payment month." rightSlot={forecast?.months.length ? <HStack spacing={2}><ForecastNav label="Previous forecast month" icon={ChevronLeft} onClick={() => scrollForecast(-1)} /><ForecastNav label="Next forecast month" icon={ChevronRight} onClick={() => scrollForecast(1)} /></HStack> : undefined}>
              {forecast?.months.length ? <Box ref={forecastCarouselRef} onPointerDown={startForecastDrag} onPointerMove={moveForecastDrag} onPointerUp={stopForecastDrag} onPointerCancel={stopForecastDrag} onLostPointerCapture={() => setIsDraggingForecast(false)} display="flex" gap={3} overflowX="auto" overflowY="hidden" pb={2} cursor={isDraggingForecast ? 'grabbing' : 'grab'} scrollSnapType={isDraggingForecast ? 'none' : 'x mandatory'} userSelect={isDraggingForecast ? 'none' : 'auto'} sx={{ WebkitOverflowScrolling: 'touch', scrollbarWidth: 'none', '&::-webkit-scrollbar': { display: 'none' } }}>{(forecast?.months ?? []).map((month) => <ForecastCard key={month.month} month={month} hasIncomePlan={forecast?.hasIncomePlan ?? false} />)}</Box> : <EmptyPlan icon={TrendingUp} title="No future cash flow yet" body="Add future transactions, recurring payments or instalments to see the monthly balance forecast." />}
            </PlanPanel>
          </MotionBox>

          {horizons.length > 0 && <><MotionBox variants={riseV}><SectionLabel>Key horizons</SectionLabel></MotionBox><SimpleGrid columns={3} spacing={{ base: '0.5rem', md: '0.8rem' }}>{horizons.map(({ label, month }) => <MotionBox key={label} variants={riseV}><Horizon label={label} month={month} /></MotionBox>)}</SimpleGrid></>}
          ...
          {false && <MotionBox variants={riseV}>
            <PlanPanel eyebrow="Projection" title="Your 12-month runway" caption={forecast?.hasProjectionBasis ? `Based on recent activity from ${basisLabel}, recurring commitments, instalments and your income assumption.` : 'Add transaction history, recurring payments or instalments to make this forecast more useful.'} rightSlot={forecast?.months.length ? <HStack spacing={2}><ForecastNav label="Previous forecast month" icon={ChevronLeft} onClick={() => scrollForecast(-1)} /><ForecastNav label="Next forecast month" icon={ChevronRight} onClick={() => scrollForecast(1)} /></HStack> : undefined}>
              {forecast?.months.length ? <Box ref={forecastCarouselRef} onPointerDown={startForecastDrag} onPointerMove={moveForecastDrag} onPointerUp={stopForecastDrag} onPointerCancel={stopForecastDrag} onLostPointerCapture={() => setIsDraggingForecast(false)} display="flex" gap={3} overflowX="auto" overflowY="hidden" pb={2} cursor={isDraggingForecast ? 'grabbing' : 'grab'} scrollSnapType={isDraggingForecast ? 'none' : 'x mandatory'} userSelect={isDraggingForecast ? 'none' : 'auto'} sx={{ WebkitOverflowScrolling: 'touch', scrollbarWidth: 'none', '&::-webkit-scrollbar': { display: 'none' } }}>{(forecast?.months ?? []).map((month) => <ForecastCard key={month.month} month={month} hasIncomePlan={forecast?.hasIncomePlan ?? false} />)}</Box> : <EmptyPlan icon={TrendingUp} title="Forecast is still collecting data" body="Scheduled commitments will appear here immediately; transaction history makes estimated income and spending more accurate." />}
            </PlanPanel>
          </MotionBox>}

          {false && horizons.length > 0 && <><MotionBox variants={riseV}><SectionLabel>Key horizons</SectionLabel></MotionBox><SimpleGrid columns={{ base: 1, md: 3 }} spacing={{ base: '0.6rem', md: '0.8rem' }}>{horizons.map(({ label, month }) => <MotionBox key={month.month} variants={riseV}><Horizon label={label} month={month} /></MotionBox>)}</SimpleGrid></>}
        </VStack>
      </MotionBox>
    </Box>
  )
...
function ForecastCard({ month, hasIncomePlan }: { month: CashFlowForecastMonth; hasIncomePlan: boolean }) {
  ...
  return <Box data-forecast-card flex={{ base: '0 0 80%', sm: '0 0 58%', lg: '0 0 calc(33.333% - 8px)' }} minW={0} p={{ base: 3, md: 4 }} borderRadius="16px" scrollSnapAlign="start" bg={current ? 'var(--pb-tint-green)' : 'var(--pb-surface-2)'} border="1px solid" borderColor={month.negative ? 'var(--pb-tint-coral)' : current ? 'var(--pb-hair-2)' : 'var(--pb-hair)'}>
    <Flex justify="space-between" align="start" gap={3}>
      <Box><Text fontWeight={600} fontSize={{ base: 'sm', md: 'md' }} color="var(--pb-ink)">{monthLabel(month.month)}</Text><Text fontFamily="var(--pb-mono)" fontSize="9px" letterSpacing="0.12em" textTransform="uppercase" color="var(--pb-ink-faint)" mt="4px">{confidence === 100 ? 'All scheduled' : hasIncomePlan ? `${confidence}% scheduled` : 'No scheduled activity'}</Text></Box>
      <Box textAlign="right"><Text className="num" fontSize={{ base: 'md', md: 'lg' }} fontWeight={600} color={colour} style={{ fontVariantNumeric: 'tabular-nums' }}>{money(month.projectedClosingBalance)}</Text><Text fontSize="xs" color={month.netCashFlow >= 0 ? 'var(--pb-income)' : 'var(--pb-coral)'} mt="2px">{signedMoney(month.netCashFlow)} net</Text></Box>
    </Flex>
    <SimpleGrid columns={2} spacing={3} mt={{ base: 3, md: 4 }}>
      <Box><Text fontFamily="var(--pb-mono)" fontSize="9px" letterSpacing="0.12em" textTransform="uppercase" color="var(--pb-ink-faint)">Income</Text><Text fontSize="sm" fontWeight={600} color="var(--pb-income)" mt="2px">{money(current ? projectedIncome : scheduledIncome + month.estimatedIncome)}</Text><Text fontSize="10px" color="var(--pb-ink-soft)" mt="2px" noOfLines={1}>{current ? 'Projected for this month' : `Scheduled${month.estimatedIncome > 0 ? ' + income plan' : ''}`}</Text></Box>
      <Box><Text fontFamily="var(--pb-mono)" fontSize="9px" letterSpacing="0.12em" textTransform="uppercase" color="var(--pb-ink-faint)">Outgoings</Text><Text fontSize="sm" fontWeight={600} color="var(--pb-coral)" mt="2px">{money(current ? projectedOutgoings : scheduledOutgoings + month.estimatedVariableExpense)}</Text><Text fontSize="10px" color="var(--pb-ink-soft)" mt="2px" noOfLines={1}>{current ? 'Paid + scheduled this month' : `Scheduled${month.estimatedVariableExpense > 0 ? ' + day-to-day estimate' : ''}`}</Text></Box>
    </SimpleGrid>
    {current && <Box mt={{ base: 3, md: 4 }} p={{ base: 2.5, md: 3 }} borderRadius="10px" bg="var(--pb-surface)">
      ...
      <Flex justify="space-between" gap={3} mt={2.5}><Text fontSize="xs" color="var(--pb-ink-soft)">Outgoings paid so far</Text><Text className="num" fontSize="xs" fontWeight={600} color="var(--pb-coral)">{money(expensesPaid)} of {money(projectedOutgoings)}</Text></Flex>
...
function Horizon({ label, month }: { label: string; month: CashFlowForecastMonth }) { return <Box p={{ base: 2, md: 4 }} borderRadius={{ base: '12px', md: '16px' }} bg="var(--pb-surface)" border="1px solid" borderColor={month.negative ? 'var(--pb-tint-coral)' : 'var(--pb-hair)'} boxShadow="0 1px 2px rgba(15,23,42,.04)"><Text fontFamily="var(--pb-mono)" fontSize={{ base: '8px', md: '9px' }} letterSpacing="0.14em" textTransform="uppercase" color="var(--pb-ink-faint)" noOfLines={1}>{label}</Text><Text className="num" fontSize={{ base: 'xs', sm: 'sm', md: 'xl' }} fontWeight={500} color={month.negative ? 'var(--pb-coral)' : 'var(--pb-ink)'} mt="4px" noOfLines={1} style={{ fontVariantNumeric: 'tabular-nums' }}>{money(month.projectedClosingBalance)}</Text><Text fontSize={{ base: '9px', md: 'xs' }} color="var(--pb-ink-soft)" mt="3px" noOfLines={1}>End of {monthLabel(month.month)}</Text></Box> }
