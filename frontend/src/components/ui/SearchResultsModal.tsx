...
export default function SearchResultsModal({ isOpen, onClose, searchFilters }: SearchResultsModalProps) {
  ...
  // Buscar transações quando o modal abre
  useEffect(() => {
  ...
  const { filteredResults, totalIncome, totalExpense, totalAmount, groupedByCategory } = useMemo(() => {
    ...
    results.forEach(t => {
      if (!grouped[t.category]) {
        grouped[t.category] = { total: 0, transactions: [] }
      }
  ...
  const getFilterDescription = () => {
    ...
    if (searchFilters.startDate && searchFilters.endDate) {
      parts.push(`${searchFilters.startDate} to ${searchFilters.endDate}`)
    } else if (searchFilters.startDate) {
      parts.push(`from ${searchFilters.startDate}`)
    } else if (searchFilters.endDate) {
      parts.push(`until ${searchFilters.endDate}`)
    }
                  ...
                  >
                    <VStack spacing={3} align="stretch">
                      ...
                      {totalAmount !== 0 && (
                        ...
                        >
                          <Box
                            w={`${Math.abs(percentage)}%`}
                            h="full"
                            bg={categoryTotal >= 0 ? 'green.400' : 'red.400'}
