...
const incomeOtherCategories = TRANSACTION_INCOME_OTHER_OPTIONS.map((name, i) => ({
  name,
  icon: (INCOME_OTHER_ICONS[i] ?? FileText) as LucideIcon,
}))

const expenseOtherCategories = TRANSACTION_EXPENSE_OTHER_OPTIONS.map((name, i) => ({
  name,
  icon: (EXPENSE_OTHER_ICONS[i] ?? FileText) as LucideIcon,
}))
...
export default function CategorySelector({
  ...
  const allCategories = useMemo<ReadonlyArray<{ name: string; icon: LucideIcon }>>(
    () => (isIncome
      ? [...incomeCategories, ...incomeOtherCategories]
      : [...expenseCategories, ...expenseOtherCategories]),
    [isIncome]
  )

  const [customDraft, setCustomDraft] = useState(() =>
    isCustomCategory(type, category) ? category.trim() : ''
  )
                ...
                return (
                  <Box key={cat.name} flexShrink={0} sx={{ scrollSnapAlign: 'start' }}>
                    <Button
                      ...
                      bg={isSelected ? colors.bgSecondary : 'transparent'}
