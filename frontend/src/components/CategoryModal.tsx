  ...
  if (sortedCategories.length === 0) {
    return (
      <Modal isOpen={isOpen} onClose={onClose} size={{ base: "full", md: "lg" }} isCentered>
        <ModalOverlay />
        <ModalContent maxH={{ base: "100vh", md: "90vh" }} m={{ base: 0, md: 4 }} display="flex" flexDirection="column">
          ...
          <VStack spacing={4} align="stretch">
            ...
            <VStack spacing={3} align="stretch">
              {sortedCategories.map(({ category, total: categoryTotal, transactions: categoryTransactions }, index) => {
                ...
                return (
                  <Box key={category} p={{ base: 3, md: 4 }} border="1px solid" borderColor={useColorModeValue('gray.200', 'gray.600')} borderRadius="lg">
                    <VStack spacing={3} align="stretch">
                      {/* Category Header */}
                      <HStack justify="space-between" align="center" wrap="wrap" gap={2}>
                      ...
                      <Progress
                        value={percentage}
                        colorScheme={color.split('#')[1] ? 'gray' : 'blue'}
                        size="lg"
                        borderRadius="md"
