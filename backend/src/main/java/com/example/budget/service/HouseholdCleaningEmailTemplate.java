...
public class HouseholdCleaningEmailTemplate {
    ...
    public EmailContent incomplete(
            String name,
            String household,
            long completed,
            int total,
            List<String> incompleteDuties) {
        long remaining = Math.max(0, total - completed);
        String plainDutyList = incompleteDuties.stream()
                .map(duty -> "- " + duty)
                .reduce("", (left, right) -> left.isEmpty() ? right : left + "\n" + right);
        String text = "Hello " + name + ",\n\nYour cleaning week for " + household
                + " ends today. You have completed " + completed + " of " + total
                + " tasks. Please make sure you complete all remaining tasks before the end of today:\n"
                + plainDutyList;
        String title = remaining == 1 ? "One task still needs you" : remaining + " tasks still need you";
        String duties = incompleteDuties.stream()
                .map(duty -> "<li style=\"margin:0 0 7px;\">" + escape(duty) + "</li>")
                .reduce("", String::concat);
        return new EmailContent(text, layout(
                "Cleaning reminder", title,
                "Hi " + escape(name) + ", your cleaning week for <strong>"
                        + escape(household) + "</strong> ends today."
                        + "<div style=\"margin-top:12px;font-weight:bold;color:#34483d;\">"
                        + "Please make sure you complete all remaining tasks before the end of today."
                        + "</div>"
                        + "<div style=\"margin-top:16px;padding:16px 18px;background:#f8faf9;border:1px solid #e4eee8;border-radius:12px;\">"
                        + "<div style=\"margin-bottom:10px;font-size:12px;font-weight:bold;color:#5b7467;text-transform:uppercase;letter-spacing:.8px;\">Still to complete</div>"
                        + "<ul style=\"margin:0;padding-left:19px;font-size:15px;line-height:1.45;color:#34483d;\">"
                        + duties + "</ul></div>",
    ...
    private String layout(String eyebrow, String title, String intro, String label, String value, String button) {
        return """
                ...
                <tr><td style="padding:28px 32px 12px;"><div style="color:#168461;font-size:12px;font-weight:bold;letter-spacing:1.3px;text-transform:uppercase;">%s</div><h1 style="margin:12px 0 16px;font-size:28px;line-height:1.2;">%s</h1><div style="margin:0;font-size:16px;line-height:1.65;color:#52645a;">%s</div></td></tr>
